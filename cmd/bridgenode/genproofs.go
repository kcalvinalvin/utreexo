...
func BuildProofs(
	...
	go pFileWorker(proofAndHeightChan, pFile, &fileWait)
	go pOffsetFileWorker(proofChan, &pOffset, pOffsetFile, &fileWait)
...
func pFileWorker(blockProofAndHeight chan util.ProofAndHeight,
	pFile *os.File, fileWait *sync.WaitGroup) {
...
func pOffsetFileWorker(proofChan chan []byte, pOffset *int32,
	pOffsetFile *os.File, fileWait *sync.WaitGroup) {
	...
	for {
		...
		*pOffset += int32(len(bp)) + int32(8) // add 8 for height bytes and load size
		...
		_, err := pOffsetFile.Write(writebyte)
		if err != nil {
			panic(err)
		}
		//counter = 0
