...
func (bp *BatchProof) Serialize(w io.Writer) (err error) {
	...
	err = freeBytes.PutUint32(w, binary.BigEndian, uint32(len(bp.Targets)))
	if err != nil {
		return err
	}
	// write out number of hashes in the proof
	err = freeBytes.PutUint32(w, binary.BigEndian, uint32(len(bp.Proof)))
	if err != nil {
		return
	}
	...
	for _, t := range bp.Targets {
		...
		err = freeBytes.PutUint64(w, binary.BigEndian, uint64(t))
		if err != nil {
			return
		}
	}
